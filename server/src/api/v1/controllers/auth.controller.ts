...
export const AuthController = {
  register: async (context: RequestContext) => {
    try {
      const validatedData = registerSchema.parse(context.body);

      const result = await context.withTransaction(async (session) => {
        const { email, password, phoneNumber, fullName } = validatedData;
  ...
  login: async (context: RequestContext) => {
    try {
      const validatedData = loginSchema.parse(context.body);

      const result = await context.withTransaction(async (session) => {
        const { email, password } = validatedData;

        // Find user by email only (as per schema)
